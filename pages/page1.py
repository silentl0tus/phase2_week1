...
# --- НАСТРОЙКИ ---
CLASSES = ['buildings', 'forest', 'glacier', 'mountain', 'sea', 'street']
# Путь к весам, который был в вашей версии 2
WEIGHTS_PATH = 'models/Max/model_weights.pth'
# Путь к данным (для аналитики)
DATASET_PATH = "/root/.cache/kagglehub/datasets/puneet6060/intel-image-classification/versions/2"
...
def load_my_model():
    model = get_resnet50_model(num_classes=len(CLASSES))
    # Загружаем веса. Если файла нет, Streamlit выдаст ошибку
    if os.path.exists(WEIGHTS_PATH):
        ...
        st.error(f"Файл весов не найден по адресу: {WEIGHTS_PATH}")
        return None
...
# --- ИНТЕРФЕЙС ---
st.set_page_config(page_title="Intel Image Classifier & Analytics", layout="wide")

# Создаем главные разделы через вкладки вверху страницы
main_tab1, main_tab2 = st.tabs(["🔍 Классификатор", "📊 Аналитика обучения"])

with main_tab1:
    st.title("Классификация Intel Images")
    st.write("Загрузите фото или вставьте ссылку на пейзаж.")

    col_input, col_viz = st.columns([1, 1])

    with col_input:
        sub_tab1, sub_tab2 = st.tabs(["Загрузить файл", "Вставить ссылку"])
        image = None
        ...
        with sub_tab2:
            url = st.text_input("Вставьте прямую ссылку на URL:")
            if url:
                try:
                    response = requests.get(url, timeout=10)
                    image = Image.open(BytesIO(response.content)).convert('RGB')
...
with main_tab2:
    st.title("Отчет об обучении")
    
    # Блок 1: Состав датасета (с проверкой на наличие папки)
    st.header("📂 Анализ исходных данных")
    if os.path.exists(DATASET_PATH):
        # ... ваш код для подсчета файлов в папках ...
        st.write("Статистика по папке:", DATASET_PATH)
    else:
        st.info("💡 Папка с исходным датасетом не найдена. Это нормально для облачного деплоя. Используются закешированные метрики обучения.")

    # Блок 2: Графики (фиксированные данные из ваших логов обучения)
    epochs = list(range(1, 11))
    train_acc = [75.2, 82.1, 85.5, 88.0, 89.2, 91.5, 92.8, 93.1, 93.5, 94.2]
    val_acc = [78.1, 83.4, 86.2, 87.5, 88.1, 89.5, 90.2, 90.8, 91.1, 91.5]
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.subheader("Точность обучения")
        fig, ax = plt.subplots()
